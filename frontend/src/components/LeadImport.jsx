                ...
                <tbody>{fileQueue.map((fq, idx) => (
                  <tr key={idx}>
                    ...
                    <td>{fq.vendor === 'Cheryl'
                      ...
                      : <select className="form-input" style={{ fontSize: '0.7rem', padding: '0.2rem 0.4rem' }} value={fq.tier} onChange={e => updateFileQueueItem(idx, { tier: e.target.value })} disabled={fq.vendor === 'Proven Leads'}>{(VENDOR_TIERS[fq.vendor] || []).map(t => <option key={t} value={t}>{t}</option>)}</select>
                    }</td>
