...
const STEP_LABELS = { source: 'Source', file: 'Upload', sheets: 'Sheets', mapping: 'Map Columns', metadata: 'Lead Details', preview: 'Preview', importing: 'Importing', results: 'Results' }

function LeadImportWizardModal({ isOpen, onClose, onComplete, getHeaders }) {
  ...
  const handleDrop = useCallback((e) => { e.preventDefault(); e.stopPropagation(); setDragActive(false); if (e.dataTransfer.files?.[0]) parseFile(e.dataTransfer.files[0]) }, [])
  ...
  const doImport = async () => {
    ...
    setStep('importing'); setError(null); setProgress({ current: 0, total: leads.length }); setImporting(true)
    ...
    for (let i = 0; i < leads.length; i += BS) {
      ...
      try {
        ...
        else { for (const l of batch) { try { const r = await fetch('/api/public/leads/capture', { method: 'POST', headers: hdrs, body: JSON.stringify(l) }); if (r.ok) created++; else failed++ } catch { failed++ } }; continue }
      } catch { failed += batch.length }
      setProgress({ current: Math.min(i + BS, leads.length), total: leads.length })
    }
    setResult({ created, failed, errors }); setStep('results'); setImporting(false)
  ...
  const titleMap = { source: 'Import Leads', file: 'Upload File', sheets: 'Google Sheets', mapping: 'Map Columns', metadata: 'Lead Details', preview: 'Review & Confirm', importing: 'Importing...', results: 'Import Complete' }
  ...
  return (
    <div style={modalBg} onClick={e => { if (e.target === e.currentTarget && step !== 'importing') onClose() }}>
      <div style={modalBox}>
        {/* Header */}
        <div style={headerBar}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
            {step !== 'source' && step !== 'importing' && step !== 'results' && (
              <button className="btn btn-sm" onClick={goBack} style={{ padding: '0.2rem 0.5rem' }}>&larr;</button>
            )}
            <span style={{ fontFamily: 'var(--font-display)', fontSize: '0.9rem', fontWeight: 600, color: 'var(--text)' }}>{titleMap[step]}</span>
            {fileName && step !== 'source' && step !== 'results' && (
              <span style={{ fontFamily: 'var(--font-mono)', fontSize: '0.65rem', color: 'var(--text-muted)' }}>{fileName}</span>
            )}
          </div>
          {step !== 'importing' && <button className="btn btn-sm" onClick={onClose} style={{ padding: '0.2rem 0.5rem' }}>&times;</button>}
        </div>

        {/* Step indicator */}
        {stepIdx >= 0 && (
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.375rem', padding: '0.5rem 1.25rem', borderBottom: '1px solid var(--border)', fontFamily: 'var(--font-mono)', fontSize: '0.6rem', color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>
            {stepFlow.map((s, i) => {
        ...
        <div style={bodyPad}>
          ...
          {step === 'source' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.625rem' }}>
              <button onClick={() => { setSourceType('file'); setStep('file') }} className="btn" style={{ display: 'flex', alignItems: 'center', gap: '1rem', padding: '1rem 1.25rem', textAlign: 'left', height: 'auto' }}>
                <div style={{ width: 40, height: 40, background: 'oklch(18% 0.04 85)', borderRadius: 3, display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--accent)', fontSize: '1.25rem', fontWeight: 700, flexShrink: 0 }}>&uarr;</div>
                <div><div style={{ fontFamily: 'var(--font-display)', fontSize: '0.85rem', fontWeight: 600, color: 'var(--text)' }}>Upload File</div><div style={{ fontFamily: 'var(--font-mono)', fontSize: '0.7rem', color: 'var(--text-muted)' }}>CSV, Excel (.xlsx, .xls), TSV</div></div>
              </button>
          ...
          {step === 'file' && (
            <div>
              <div onDragEnter={handleDrag} onDragLeave={handleDrag} onDragOver={handleDrag} onDrop={handleDrop} onClick={() => fileInputRef.current?.click()}
                style={{ border: '2px dashed ' + (dragActive ? 'var(--accent)' : 'var(--border)'), borderRadius: 3, padding: '3rem 1.5rem', textAlign: 'center', cursor: 'pointer', background: dragActive ? 'oklch(14% 0.015 85 / 0.1)' : 'var(--bg)', transition: 'all 0.15s' }}>
                ...
                <div style={{ fontSize: '2rem', marginBottom: '0.5rem', color: 'var(--text-muted)' }}>&uarr;</div>
                <p style={{ fontFamily: 'var(--font-mono)', fontSize: '0.8rem', color: 'var(--text)' }}>Drop files here or click to browse</p>
                <p className="form-hint" style={{ margin: '0.375rem 0 0' }}>CSV, Excel (.xlsx, .xls), TSV</p>
              </div>
            </div>
          )}
          ...
          {step === 'sheets' && (
            <div>
              ...
              <p className="form-hint" style={{ margin: '0.75rem 0 0' }}>If Google Sheets API is not configured, export as CSV and use file upload.</p>
            </div>
          )}
          ...
          {step === 'mapping' && (
            <div>
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '1rem' }}>
                <button className="btn btn-primary" onClick={() => setStep('metadata')} disabled={!mappingOk}>Next &rarr;</button>
              </div>
            </div>
          )}
          ...
          {step === 'metadata' && (
            <div>
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '1rem', gap: '0.5rem' }}>
                <button className="btn" onClick={() => setStep('mapping')}>&larr; Back</button>
                <button className="btn btn-primary" onClick={() => setStep('preview')}>Review &rarr;</button>
              </div>
            </div>
          )}
          ...
          {step === 'preview' && (
            <div>
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '1rem', gap: '0.5rem' }}>
                <button className="btn" onClick={() => setStep('metadata')}>&larr; Back</button>
          ...
          {step === 'results' && result && (
            <div>
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '0.5rem' }}>
                <button className="btn" onClick={() => { setStep('source'); setResult(null); setError(null); setFileName(''); setHeaders([]); setParsedRows([]); setColumnMap({}) }}>Import More</button>
...
export default LeadImportWizardModal
